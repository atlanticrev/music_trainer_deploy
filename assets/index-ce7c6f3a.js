...
const createElFromSchema = (schema) => {
  ...
  if (schema.events) {
    for (const [eventName, handler2] of Object.entries(schema.events)) {
      if (typeof handler2 === "function") {
        element.addEventListener(eventName, handler2);
      }
    }
  }
  if (schema.children) {
    if (element.shadowRoot) {
      element.shadowRoot.append(createElTreeFromSchema(schema.children));
    } else {
...
const getTagNameByCtor = (ctor) => {
  const name = ctor.name.split(/(?=[A-Z])|\b/).map((word) => word.toLowerCase()).join("-");
  return `x-${name}`;
};
...
class CustomElement extends HTMLElement {
  ...
  applyTemplate() {
    const node = createElTreeFromSchema(this.getTemplateSchema());
    if (node) {
...
class SidePanel extends CustomElement {
  constructor() {
    ...
    __publicField(this, "closeSidePanel", () => {
      ...
      Promise.all([
        ...
        this._sidePanelContainer.style.setProperty("display", "none");
      });
    });
    this._menuOpenButton = document.querySelector(".menu-btn-open");
    this._menuCloseButton = this.shadowRoot.querySelector(".menu-btn-close");
    this._sidePanelContainer = this.shadowRoot.querySelector(".side-panel-container");
    this._sidePanelOverlay = this.shadowRoot.querySelector(".side-panel-overlay");
    this._sidePanel = this.shadowRoot.querySelector(".side-panel");
    this.toggleSidePanel = this.toggleSidePanel.bind(this);
  }
  connectedCallback() {
    this._menuOpenButton.addEventListener("click", this.toggleSidePanel);
    this._menuCloseButton.addEventListener("click", this.toggleSidePanel);
    this._sidePanelOverlay.addEventListener("click", this.toggleSidePanel);
  }
  getTemplateSchema() {
    return {
      tagName: "div",
      cssClasses: ["side-panel-container"],
      children: [
        {
          tagName: "div",
          cssClasses: ["side-panel-overlay"]
        },
        {
          tagName: "div",
          cssClasses: ["side-panel"],
          children: [
            {
              tagName: "div",
              cssClasses: ["side-panel-header"],
              children: [
                {
                  tagName: "button",
                  cssClasses: ["menu-btn-close"],
                  children: [
                    {
                      tagName: "x-menu-icon"
                    }
      ...
      },
      {
        ...
        events: {
          click: (e) => {
            ...
            dialog.setContent(createElTreeFromSchema({
              tagName: "x-button-group-multiple",
              cssClasses: "grid-button-group",
...
const styles$2 = ".menu-btn-close, .menu-btn-open {\n  margin: 0;\n  padding: 0;\n  border: 0;\n  outline: 0;\n  box-sizing: border-box;\n}\n\n.menu-btn-close, .menu-btn-open, .neu-button {\n  user-select: none;\n}\n\n.menu-btn-close:hover, .menu-btn-open:hover, .neu-button:hover {\n  cursor: pointer;\n}\n\n.neu-button {\n  width: 115px;\n  height: 115px;\n  border: 1px solid #373a3f;\n  border-radius: 20px;\n  background: linear-gradient(-45deg, #26272b, #33363b);\n  color: #ffffff;\n  font-weight: bolder;\n  font-size: 1.05rem;\n}\n\n.neu-button:active {\n  background: linear-gradient(-45deg, #33363b, #26272b);\n  box-shadow: -8px -8px 2em #33363b, 15px 15px 1.5em #26272b;\n}\n\n.menu-btn-close, .menu-btn-open {\n  background-color: transparent;\n}\n\n.neu-button {\n  width: 80px;\n  height: 80px;\n  border: 1px solid #373a3f;\n  border-radius: 20px;\n  background: linear-gradient(-45deg, #26272b, #33363b);\n  color: #ffffff;\n  font-weight: bolder;\n}\n\n.neu-button:active {\n  background: linear-gradient(-45deg, #33363b, #26272b);\n  box-shadow: -8px -8px 2em #33363b, 15px 15px 1.5em #26272b;\n}\n\n:host {\n  gap: 0.5rem;\n}\n\n:host > .grid-cell > button {\n  font-size: 1.2rem;\n}\n\n@media screen and (max-width: 420px) {\n  :host > .grid-cell > button {\n    width: 100%;\n  }\n}";
...
class Dialog extends CustomElement {
  ...
  connectedCallback() {
    this._menuCloseButton = this.shadowRoot.querySelector(".dialog-close-button");
    ...
    this._slot = this.shadowRoot.querySelector('slot[name="dialog-content"]');
  }
...
const initSidePanel = (config) => {
  ...
  const openSidePanel = () => {
    ...
    sidePanelContainer.style.setProperty("display", "block");
    Promise.all([
      TransitionAnimator.animate({
        target: sidePanelOverlay,
        property: "opacity",
        time: "0.25s",
        easing: "ease-out",
        from: "0",
  ...
  const tonalityControl = createElFromSchema({
    tagName: "div",
    cssClasses: ["control"],
    children: [
      {
